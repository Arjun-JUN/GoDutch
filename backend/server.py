...
async def register(user_data: UserRegister):
    ...
    token = jwt.encode(
        {"user_id": user_id, "email": user_data.email, "exp": datetime.now(UTC) + timedelta(days=30)},
        JWT_SECRET,
        algorithm='HS256'
    )
...
async def login(credentials: UserLogin):
    ...
    token = jwt.encode(
        {"user_id": user_doc['id'], "email": user_doc['email'], "exp": datetime.now(UTC) + timedelta(days=30)},
        JWT_SECRET,
        algorithm='HS256'
    )
